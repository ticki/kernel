...
use arch::paging::{ActivePageTable, InactivePageTable, Page, PageIter, PhysicalAddress,
                   VirtualAddress};
        ...
        where F: FnOnce(&mut Memory) -> T
    {
        match *self {
            ...
            SharedMemory::Borrowed(ref memory_weak) => {
                let memory_lock = memory_weak.upgrade()
...
impl Memory {
    pub fn new(start: VirtualAddress,
               size: usize,
               flags: EntryFlags,
               flush: bool,
               clear: bool)
    ...
    fn map(&mut self, flush: bool, clear: bool) {
        ...
        if clear {
            ...
            unsafe {
                memset(self.start_address().get() as *mut u8, 0, self.size);
            }
        }
    }
    ...
    pub fn resize(&mut self, new_size: usize, flush: bool, clear: bool) {
        ...
        if new_size > self.size {
            ...
            if clear {
                assert!(flush);
                unsafe {
                    memset((self.start.get() + self.size) as *mut u8,
                           0,
                           new_size - self.size);
                }
            }
