...
impl Scheme for DisplayScheme {
    fn open(&self, path: &[u8], _flags: usize) -> Result<usize> {
        if path == b"input" { Ok(1) } else { Ok(0) }
    }
...
fn main() {
...

    {
        let mode_info = unsafe {
            &*(physmap(0x5200, 4096, 0)
                .expect("vesad: failed to map VBE \
                         info") as *const VBEModeInfo)
        };
        ...
        unsafe {
            let _ = physunmap(mode_info as *const _ as usize);
        }
    }

    if physbaseptr > 0 {
        thread::spawn(move || {
            let mut socket = File::create(":display")
                .expect("vesad: failed to create display scheme");

            let size = width * height;

            let onscreen = unsafe {
                physmap(physbaseptr as usize,
                        size * 4,
                        MAP_WRITE | MAP_WRITE_COMBINE)
                    .expect("vesad: failed to map VBE LFB")
            };
            ...
            let scheme = DisplayScheme {
                ...
                display: RefCell::new(Display::new(width,
                                                   height,
                                                   unsafe {
            ...
            loop {
                ...
                if packet.a == syscall::number::SYS_READ && packet.d > 0 &&
                   scheme.input.borrow().is_empty() {
                ...
                if !scheme.input.borrow().is_empty() &&
                   *scheme.requested.borrow() & EVENT_READ == EVENT_READ {
